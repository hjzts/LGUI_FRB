...
#         timeout=15
#     )
#     res = completion.choises[0].message.content
#     return res
...
def query_gpt(prompt):
    ...
    client = OpenAI(
        ...
        api_key='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',
    )
    retry = 0
...
def get_thought(answer):
    ...
    substring = answer[start_index:end_index] if end_index != - \
        1 else answer[start_index:]
    return substring
...
def extract_action(v):
    ...
    try:
        if 'Finished' in v.keys():
            whether_finished_answer = v['Finished'].lower() == 'yes' or v['Finished'].lower(
            ) == 'y' or v['Finished'].lower() == 'true' or 'finished' in v['Finished'].lower()
...
def insert_onclick_into_prompt(state_prompt, insert_ele, target_ele_desc):
    ...
    def insert_onclick(statement, description):
        ...
        inserted_statement = statement[:index] + \
            f" onclick='{description}'" + statement[index:]
